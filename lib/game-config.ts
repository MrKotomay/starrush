...
const envSchema = z.object({
  ...
  RISK_MAX_PAYOUT_FRACTION_PER_BET: z.coerce.number().gt(0).lte(1).default(0.25),
  RISK_MAX_EXPOSURE_FRACTION_PER_ROUND: z.coerce.number().gt(0).lte(1).default(0.75),
  RISK_ACCEPTANCE_MAX_MULTIPLIER: z.coerce.number().min(1.01).default(5),
...
export function loadGameConfig(source: Record<string, string | undefined> = process.env) {
  ...
  return {
    ...
    riskAcceptanceMaxMultiplier: env.RISK_ACCEPTANCE_MAX_MULTIPLIER,
